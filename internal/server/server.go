...
func NewRouter(app *app.App, log *slog.Logger, s *store.Store, jwtKey string) http.Handler {
	...
	router.Group(func(r chi.Router) {
		...
		// Assignment endpoints (Phase 1.5 — replacing Games)
		r.Route("/assignment", func(r chi.Router) {
			r.Get("/", app.AssignmentGetPublished)
			r.Post("/", app.AssignmentCreate)
			r.Get("/my", app.AssignmentGetMy)
			r.Get("/submissions/my", app.AssignmentGetMySubmissions)
			r.Get("/stats/my", app.AssignmentGetStudentStats)
			r.Get("/{id}", app.AssignmentGetByID)
			r.Put("/{id}", app.AssignmentUpdate)
			r.Delete("/{id}", app.AssignmentDelete)
			r.Post("/{id}/submit", app.AssignmentSubmit)
			r.Get("/{id}/submissions", app.AssignmentGetSubmissions)
		})
	})
